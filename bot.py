...
def webhook_cryptobot():
    try:
        ...
        if payload and payload.get('status') == 'paid':
            ...
            logging.warning(f"Invoice {invoice_id} not found in memory")
...
def user_has_offer_in_category(user_id, category):
    """Проверяет есть ли уже объявление у пользователя в этой категории"""
    ...
    if data == "board":
        ...
        if offer_id in offers_storage:
            ...
            if offer['author_id'] == user_id:
                ...
                try:
                    await query.edit_message_text("❌ Это не ваше объявление!", reply_markup=get_main_menu())
                except BadRequest as e:
                    if "Message is not modified" not in str(e):
                        raise e

...
def create_cryptobot_invoice(amount_usd: float, description: str, user_id: int):
    try:
        ...
        logger.info(f"🔵 Creating invoice: amount={amount_usd}, user={user_id}")
        ...
        if response.status_code == 200:
            ...
            if data.get('ok'):
                ...
                logger.info(f"✅ Invoice created: {invoice_id}, URL: {pay_url}")
                ...
                logger.error(f"❌ API returned ok=False: {data}")
        else:
